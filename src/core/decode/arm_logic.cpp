#include "cpu.hpp"
#include "instructions/arm.hpp"
#include "instructions/instruction.hpp"

#define UNDEFINED_MASK 0x6000010
#define DATA_PROC_OPCODE_MASK 0x1e00000

InstructionInfo ARM7TDMI::arm_decode(InstructionInfo& instr) {
  instr.condition = (Condition)(instr.opcode >> 28);

  if ((instr.opcode & 0xfc000f0) == 0x90) {  // MUL, MLA (Multiply)
    // spdlog::debug("read instruction {:#010X} is a multiply", instr.opcode);

    instr.S = (instr.opcode & 1 << 20) ? 1 : 0;

    instr.Rd = (instr.opcode & 0xf0000) >> 16;
    instr.Rn = (instr.opcode & 0xf000) >> 12;
    instr.Rs = (instr.opcode & 0xf00) >> 8;
    instr.Rm = (instr.opcode & 0xf);

    u8 op = (instr.opcode & 0x1e00000) >> 21;

    switch (op) {
      case 0: {  // MUL
        // instr.mnemonic = fmt::format("mul r{}, r{}, r{}, r{}", +instr.Rd, +instr.Rm, +instr.Rs, +instr.Rn);
        instr.func_ptr = ARM::Instructions::MUL;
        break;
      }
      case 1: {  // MLA
        // instr.mnemonic = fmt::format("mla r{}, r{}, r{}, r{}", +instr.Rd, +instr.Rm, +instr.Rs, +instr.Rn);
        instr.func_ptr = ARM::Instructions::MLA;
        break;
      }

      default: {
        assert(0);
      }
    }
    return instr;
  }

  if ((instr.opcode & 0xf8000f0) == 0x800090) {  // MULL, MLAL (Multiply Long)

    // spdlog::debug("read instruction {:#010X} is a multiply long", instr.opcode);
    instr.S = (instr.opcode & 1 << 20) ? 1 : 0;

    instr.Rd = (instr.opcode & 0xf0000) >> 16;
    instr.Rn = (instr.opcode & 0xf000) >> 12;
    instr.Rs = (instr.opcode & 0xf00) >> 8;
    instr.Rm = (instr.opcode & 0xf);
    u8 op    = (instr.opcode & 0x1e00000) >> 21;

    switch (op) {
      case 4: {  // UMULL
        // instr.mnemonic = fmt::format("umull{}{} r{}, r{}, r{}, r{}",instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rn, +instr.Rd, +instr.Rm, +instr.Rs);
        instr.func_ptr = ARM::Instructions::UMULL;

        break;
      }
      case 5: {  // UMLAL
        // instr.mnemonic = fmt::format("umlal{}{} r{} r{} r{} r{}", instr.S ? "s" : "",condition_map.at(instr.condition), +instr.Rn, +instr.Rd, +instr.Rm, +instr.Rs);
        instr.func_ptr = ARM::Instructions::UMLAL;
        break;
      }
      case 6: {  // SMULL
        // instr.mnemonic = fmt::format("smull{}{} r{}, r{}, r{}, r{}", instr.S ? "s" : "",condition_map.at(instr.condition), instr.S ? "s" : "", +instr.Rn, +instr.Rd, +instr.Rm, +instr.Rs);
        instr.func_ptr = ARM::Instructions::SMULL;
        break;
      }
      case 7: {  // SMLAL
        // instr.mnemonic = fmt::format("smlal{}{} r{}, r{}, r{}, r{}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rn, +instr.Rd, +instr.Rm, +instr.Rs);
        instr.func_ptr = ARM::Instructions::SMLAL;
        break;
      }
      default: {
        assert(0);
      }
    }
    return instr;
  }

  if ((instr.opcode & (0xfb00ff0)) == 0x1000090) {  // SWP (Single Data Swap)

    // spdlog::debug("read instruction {:#010X} is a single data swap", instr.opcode);

    instr.B  = (instr.opcode & (1 << 22)) ? 1 : 0;
    instr.Rn = (instr.opcode & 0xf0000) >> 16;
    instr.Rd = (instr.opcode & 0xf000) >> 12;
    instr.Rm = instr.opcode & 0xf;
    // if(instr.Rm == 15) continue;

    // assert(instr.Rn != 15);

    // instr.mnemonic = fmt::format("swp{}{} r{}, r{}, [r{}]", instr.B ? "b" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rm, +instr.Rn);
    instr.func_ptr = ARM::Instructions::SWP;

    return instr;
  }

  if ((instr.opcode & (0xe0000f0)) == 0xb0) {  // LDRH, STRH

    instr.L      = (instr.opcode & (1 << 20)) ? 1 : 0;  // Store or Load
    instr.Rn     = (instr.opcode & 0xf0000) >> 16;      // base register
    instr.Rd     = (instr.opcode & 0xf000) >> 12;       // destination register
    instr.U      = (instr.opcode & (1 << 23)) ? 1 : 0;  // UP DOWN BIT
    instr.P      = (instr.opcode & (1 << 24)) ? 1 : 0;  // indexing bit
    instr.W      = (instr.opcode & (1 << 21)) ? 1 : 0;  // writeback bit
    instr.I      = (instr.opcode & (1 << 22)) ? 1 : 0;  // Immediate bit
    instr.Rm     = (instr.opcode & 0xf);
    instr.offset = (((instr.opcode & 0xf00) >> 8) << 4) + (instr.opcode & 0xf);

    if (instr.L) {
      instr.func_ptr = ARM::Instructions::LDRH;
      if (instr.I == 0) {
        // instr.mnemonic = fmt::format("ldrh{} r{}, [r{}, r{}]", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.Rm);
      } else {
        // instr.mnemonic = fmt::format("ldrh{} r{}, [r{}], #{}{}", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.U ? "+" : "-", instr.offset);
      }

    } else {
      instr.func_ptr = ARM::Instructions::STRH;
      if (instr.I == 0) {
        // instr.mnemonic = fmt::format("strh{} r{}, [r{}, r{}]", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.Rm);
      } else {
        // instr.mnemonic = fmt::format("strh{} r{}, [r{}], #{}{}", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.U ? "+" : "-", instr.offset);
      }
    }

    // SPDLOG_INFO(instr.mnemonic);

    return instr;
  }

  if ((instr.opcode & (0xe1000d0)) == 0x1000d0) {  // LDRSB, LDRSH (Halfword and Signed Data Transfer)
    // spdlog::debug("read instruction {:#010X} is a signed data transfer", instr.opcode);

    // spdlog::debug("read instruction {:#010X} is a halfword data transfer",
    //               instr.opcode);

    instr.L      = (instr.opcode & (1 << 20)) ? 1 : 0;  // Store or Load
    instr.Rn     = (instr.opcode & 0xf0000) >> 16;      // base register
    instr.Rd     = (instr.opcode & 0xf000) >> 12;       // destination register
    instr.U      = (instr.opcode & (1 << 23)) ? 1 : 0;  // UP DOWN BIT
    instr.P      = (instr.opcode & (1 << 24)) ? 1 : 0;  // indexing bit
    instr.W      = (instr.opcode & (1 << 21)) ? 1 : 0;  // writeback bit
    instr.I      = (instr.opcode & (1 << 22)) ? 1 : 0;  // Immediate bit
    instr.Rm     = (instr.opcode & 0xf);
    instr.offset = (((instr.opcode & 0xf00) >> 8) << 4) + (instr.opcode & 0xf);

    u8 opcode = (instr.opcode & 0b1100000) >> 5;

    assert(opcode != 1);
    assert(instr.L == 1);

    // SPDLOG_DEBUG("LDRSB/LDRSH");
    std::string mode_string;

    switch (opcode) {
      case 2: {
        mode_string    = "sb";
        instr.func_ptr = ARM::Instructions::LDRSB;
        break;
      }
      case 3: {
        mode_string    = "sh";
        instr.func_ptr = ARM::Instructions::LDRSH;
        break;
      }
      default: assert("invalid opcode");
    }

    if (instr.I) {
      // instr.mnemonic = fmt::format("ldr{}{} r{}, [r{}], #{}{}", mode_string, condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.U ? "+" : "-", instr.offset);
    } else {
      // instr.mnemonic = fmt::format("ldr{}{} r{}, [r{}, {}r{}]{}", mode_string, condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.U ? "+" : "-", +instr.Rm, instr.W ? "!" : "");
    }

    return instr;
  }

  if ((instr.opcode & 0xfb000f0) == 0x1000000) {  // MRS

    instr.I = (instr.opcode & (1 << 25)) ? 1 : 0;
    instr.P = (instr.opcode & (1 << 22)) ? 1 : 0;

    assert(instr.I == 0);

    instr.Rd = (instr.opcode & 0xf000) >> 12;

    instr.func_ptr = ARM::Instructions::MRS;
    // instr.mnemonic = fmt::format("mrs{} r{}, {}", condition_map.at(instr.condition), +instr.Rd, instr.P == 0 ? "cpsr" : "spsr");
    return instr;
  }

  if ((instr.opcode & 0xfb000f0) == 0x1200000) {  // MSR [register]

    instr.P  = (instr.opcode & 0x400000) ? 1 : 0;
    instr.Rm = (instr.opcode & 0xf);
    instr.I  = (instr.opcode & (1 << 25)) ? 1 : 0;

    // bool modify_flag_field      = (instr.opcode & (1 << 19)) ? true : false;
    // bool modify_status_field    = (instr.opcode & (1 << 18)) ? true : false;
    // bool modify_extension_field = (instr.opcode & (1 << 17)) ? true : false;
    // bool modify_control_field   = (instr.opcode & (1 << 16)) ? true : false;

    instr.func_ptr = ARM::Instructions::MSR;
    // instr.mnemonic = fmt::format("msr{} {}_{}{}{}{}, r{}", condition_map.at(instr.condition), instr.P == 0 ? "cpsr" : "spsr", modify_flag_field ? "f" : "", modify_status_field ? "s" : "", modify_extension_field ? "x" : "", modify_control_field ? "c" : "", +instr.Rm);
    return instr;
  }

  if ((instr.opcode & 0xfb00000) == 0x3200000) {  // MSR immediate
    // SPDLOG_INFO("MSR [immediate]");

    instr.P         = (instr.opcode & (1 << 22)) ? 1 : 0;
    instr.I         = (instr.opcode & (1 << 25)) ? 1 : 0;
    u32 amount      = (instr.opcode & 0xff);
    u8 shift_amount = (instr.opcode & 0b111100000000) >> 8;

    amount = std::rotr(amount, shift_amount * 2);

    bool modify_flag_field      = (instr.opcode & (1 << 19));
    bool modify_status_field    = (instr.opcode & (1 << 18));
    bool modify_extension_field = (instr.opcode & (1 << 17));
    bool modify_control_field   = (instr.opcode & (1 << 16));

    std::string modifier_string = fmt::format("_{}{}{}{}", modify_flag_field ? "f" : "", modify_status_field ? "s" : "", modify_extension_field ? "x" : "", modify_control_field ? "c" : "");

    // instr.mnemonic = fmt::format("msr{} {}{}, #{:#x}", condition_map.at(instr.condition), instr.P == 0 ? "cpsr" : "spsr", modifier_string, amount);
    instr.func_ptr = ARM::Instructions::MSR;
    return instr;
  }

  if ((instr.opcode & 0xffffff0) == 0x12fff10) {  // BX
    instr.Rn = instr.opcode & 0xf;

    instr.func_ptr = ARM::Instructions::BX;
    // instr.mnemonic = fmt::format("bx r{} [{:#010x}]", +instr.Rn, regs.r[instr.Rn]);
    return instr;
  }

  if ((instr.opcode & 0xe000010) == 0) {  // Data Processing (immediate shift)

    u32 data_proc_opcode = ((instr.opcode & DATA_PROC_OPCODE_MASK) >> 21);

    instr.Rd = (instr.opcode & 0xf000) >> 12;
    instr.S  = (instr.opcode & 0x100000) ? 1 : 0;
    instr.Rm = instr.opcode & 0xf;
    instr.Rn = (instr.opcode & 0xf0000) >> 16;
    instr.I  = (instr.opcode & (1 << 25)) ? 1 : 0;
    // auto r = (instr.opcode & (1 << 4)) ? 1 : 0;
    // fmt::println(" sbr flag: {}", r);
    instr.R = false;

    assert(instr.I == 0);

    instr.shift_type   = (instr.opcode & 0x60) >> 5;
    instr.shift_amount = (instr.opcode & 0xf80) >> 7;

    std::string shift_string = get_shift_string(instr.shift_type);
    assert((instr.opcode & (1 << 4)) == 0);
    // assert(0);
    switch (data_proc_opcode) {
      case AND: {
        instr.func_ptr = ARM::Instructions::AND;
        // instr.mnemonic = fmt::format("and{}{} r{}, r{}, {} #{}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, shift_string, instr.shift_amount);
        break;
      }
      case EOR: {
        instr.func_ptr = ARM::Instructions::EOR;
        // instr.mnemonic = fmt::format("eor{}{} r{}, r{}, r{} {} #{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, instr.shift_amount);
        break;
      }
      case SUB: {
        instr.func_ptr = ARM::Instructions::SUB;
        // instr.mnemonic = fmt::format("sub{}{} r{}, r{}, r{} {} #{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, instr.shift_amount);
        break;
      }
      case RSB: {
        instr.func_ptr = ARM::Instructions::RSB;
        // instr.mnemonic = fmt::format("rsb{}{} r{}, r{}, r{} {} #{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, instr.shift_amount);
        break;
      }
      case ADD: {
        instr.func_ptr = ARM::Instructions::ADD;
        // instr.mnemonic = fmt::format("add{}{} r{}, r{}, r{} {} #{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, instr.shift_amount);
        break;
      }
      case ADC: {
        instr.func_ptr = ARM::Instructions::ADC;
        // instr.mnemonic = fmt::format("adc{}{} r{}, r{}, r{} {} #{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, instr.shift_amount);

        break;
      }
      case SBC: {
        instr.func_ptr = ARM::Instructions::SBC;
        // instr.mnemonic = fmt::format("sbc{}{} r{}, r{}, r{} {} #{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, instr.shift_amount);
        break;
      }
      case RSC: {
        instr.func_ptr = ARM::Instructions::RSC;
        // instr.mnemonic = fmt::format("rsc{}{} r{}, r{}, r{} {} #{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, instr.shift_amount);

        break;
      }
      case TST: {
        instr.func_ptr = ARM::Instructions::TST;
        // instr.mnemonic = fmt::format("tst{}{} r{}, r{}, r{} {} #{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, instr.shift_amount);
        break;
      }
      case TEQ: {
        instr.func_ptr = ARM::Instructions::TEQ;
        // instr.mnemonic = fmt::format("teq{} r{}, r{}", condition_map.at(instr.condition), +instr.Rn, +instr.Rm);
        break;
      }
      case CMP: {
        instr.func_ptr = ARM::Instructions::CMP;
        // instr.mnemonic = fmt::format("cmp r{}, r{} {} #{:#x}", instr.Rn, instr.Rm, shift_string, instr.shift_amount);
        break;
      }
      case CMN: {
        instr.func_ptr = ARM::Instructions::CMN;
        // instr.mnemonic = fmt::format("cmn{} r{}, r{}", condition_map.at(instr.condition), +instr.Rn, +instr.Rm, shift_string);
        break;
      }
      case ORR: {
        instr.func_ptr = ARM::Instructions::ORR;
        // instr.mnemonic = fmt::format("orr{}{} r{}, r{}, r{} {} #{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, instr.shift_amount);
        // instr.print_params();
        break;
      }
      case MOV: {
        instr.func_ptr = ARM::Instructions::MOV;
        // instr.mnemonic = fmt::format("mov{}{} r{}, r{}, {} #{}", condition_map.at(instr.condition), instr.S ? "s" : "", +instr.Rd, +instr.Rm, shift_string, instr.shift_amount);
        break;
      }
      case BIC: {
        instr.func_ptr = ARM::Instructions::BIC;
        // instr.mnemonic = fmt::format("bic{}{} r{}, r{}, {} #{}", condition_map.at(instr.condition), instr.S ? "s" : "", +instr.Rd, +instr.Rm, shift_string, instr.shift_amount);

        break;
      }
      case MVN: {
        instr.func_ptr = ARM::Instructions::MVN;
        // instr.mnemonic = fmt::format("mvn{}{} r{}, r{}, {} #{}", condition_map.at(instr.condition), instr.S ? "s" : "", +instr.Rd, +instr.Rm, shift_string, instr.shift_amount);

        break;
      }

      default: {
        // SPDLOG_ERROR("could not resolve data processing opcode: {:08b}", data_proc_opcode);
        assert(0);
      }
    }

    return instr;
  }

  if ((instr.opcode & 0xe000090) == 0x10) {  // shift value is a register
    // fmt::println("Data Processing (shift value is a register)");
    u32 o_opcode = ((instr.opcode & DATA_PROC_OPCODE_MASK) >> 21);
    instr.Rd     = (instr.opcode & 0xf000) >> 12;
    instr.S      = (instr.opcode & (1 << 20)) ? 1 : 0;
    instr.Rm     = instr.opcode & 0xf;
    instr.Rn     = (instr.opcode & 0xf0000) >> 16;
    instr.Rs     = (instr.opcode & 0xf00) >> 8;

    instr.R          = true;
    instr.shift_type = (instr.opcode & 0x60) >> 5;

    // u8 shift_amount  = regs.r[instr.Rs] & 0xFF;

    SPDLOG_DEBUG("OPCODE: {:#010x}", instr.opcode);
    std::string shift_string = get_shift_string(instr.shift_type);

    instr.I = (instr.opcode & (1 << 25)) != 0 ? 1 : 0;

    assert(instr.I == 0);

    switch (o_opcode) {
      case AND: {
        instr.func_ptr = ARM::Instructions::AND;
        // instr.mnemonic = fmt::format("and{}{} r{}, r{}, r{} {} r{} [{}]", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, +instr.Rs, shift_amount);

        break;
      }
      case EOR: {
        instr.func_ptr = ARM::Instructions::EOR;
        // instr.mnemonic = fmt::format("eor{}{} r{}, r{}, r{} {} r{} [{}]", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, +instr.Rs, shift_amount);
        break;
      }
      case SUB: {
        instr.func_ptr = ARM::Instructions::SUB;
        // instr.mnemonic = fmt::format("sub{}{} r{}, r{}, r{} {} r{} [{}]", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, +instr.Rs, shift_amount);
        break;
      }
      case RSB: {
        instr.func_ptr = ARM::Instructions::RSB;
        // instr.mnemonic = fmt::format("rsb{}{} r{}, r{}, r{} {} r{} [{}]", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, +instr.Rs, shift_amount);
        break;
      }
      case ADD: {
        instr.func_ptr = ARM::Instructions::ADD;
        // instr.mnemonic = fmt::format("add{}{} r{}, r{}, r{} {} r{} [{}]", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, +instr.Rs, shift_amount);
        break;
      }
      case ADC: {
        instr.func_ptr = ARM::Instructions::ADC;
        // instr.mnemonic = fmt::format("adc{}{} r{}, r{}, r{} {} r{} [{}]", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, +instr.Rs, shift_amount);
        break;
      }
      case SBC: {
        instr.func_ptr = ARM::Instructions::SBC;
        // instr.mnemonic = fmt::format("sbc{}{} r{}, r{}, r{} {} r{} [{}]", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, +instr.Rs, shift_amount);
        break;
      }
      case RSC: {
        instr.func_ptr = ARM::Instructions::RSC;
        // instr.mnemonic = fmt::format("rsc{}{} r{}, r{}, r{} {} r{}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, +instr.Rs);
        break;
      }
      case TST: {
        instr.func_ptr = ARM::Instructions::TST;
        // instr.mnemonic = fmt::format("tst{}{} r{}, r{} {} r{} [{}]", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rn, +instr.Rm, shift_string, +instr.Rs, shift_amount);
        break;
      }
      case TEQ: {
        instr.func_ptr = ARM::Instructions::TEQ;
        // instr.mnemonic = fmt::format("teq{}{} r{}, r{}, {} r{}", condition_map.at(instr.condition), instr.S ? "s" : "", +instr.Rd, +instr.Rn, shift_string, +instr.Rs);
        break;
      }
      case CMP: {
        instr.func_ptr = ARM::Instructions::CMP;
        // instr.mnemonic = fmt::format("cmp{}{} r{}, r{}, r{} {} r{} [{}]", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, +instr.Rs, shift_amount);
        break;
      }
      case CMN: {
        instr.func_ptr = ARM::Instructions::CMN;
        // instr.mnemonic = fmt::format("cmn{} r{}, r{} {} r{} [{}]", condition_map.at(instr.condition), +instr.Rn, +instr.Rm, shift_string, +instr.Rs, shift_amount);
        break;
      }
      case ORR: {
        instr.func_ptr = ARM::Instructions::ORR;
        // instr.mnemonic = fmt::format("orr{}{} r{}, r{}, r{} {} r{} [{}]", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, +instr.Rs, shift_amount);
        // assert(0);
        break;
      }
      case MOV: {
        instr.func_ptr = ARM::Instructions::MOV;
        // instr.mnemonic = fmt::format("mov{}{} r{}, r{}, {} r{} [{}]", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rm, shift_string, +instr.Rs, shift_amount);
        break;
      }
      case BIC: {
        instr.func_ptr = ARM::Instructions::BIC;
        // instr.mnemonic = fmt::format("bic{}{} r{}, r{}, {} r{} [{}]", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rm, shift_string, +instr.Rs, shift_amount);
        break;
      }
      case MVN: {
        instr.func_ptr = ARM::Instructions::MVN;
        // instr.mnemonic = fmt::format("mvn{}{} r{}, r{}, {} r{}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, +instr.Rm, shift_string, +instr.Rs);
        break;
      }

      default: {
        // spdlog::error("could not resolve data processing opcode: {:08b}", o_opcode);
        assert(0);
      }
    }

    return instr;
  }

  if ((instr.opcode & 0xfb00000) == 0x3000000) {  // undefined instructions
    SPDLOG_DEBUG("undefined instructions");
    // assert(0);
    return instr;
    // assert(0);
    // return instr;
  }

  if ((instr.opcode & 0xe000000) == 0x2000000) {  // Data Processing (immediate value)
    // fmt::println("Data Processing (immediate value)");

    u32 o_opcode = ((instr.opcode & DATA_PROC_OPCODE_MASK) >> 21);
    instr.Rd     = (instr.opcode & 0xf000) >> 12;
    instr.S      = (instr.opcode & 0x100000) ? 1 : 0;
    instr.Rm     = instr.opcode & 0xf;
    instr.Rn     = (instr.opcode & 0xf0000) >> 16;
    instr.imm    = instr.opcode & 0xff;
    instr.rotate = ((instr.opcode & 0xf00) >> 8);

    instr.I = (instr.opcode & 0x2000000) ? 1 : 0;

    assert(instr.I == 1);
    instr.op2 = std::rotr(instr.imm, instr.rotate * 2);
    // instr.print_params();
    // fmt::println("immediate value: {}, rot amount = {}", imm, rotate);
    // instr.op2    = instr.opcode & 0xfff;

    switch (o_opcode) {
      case AND: {
        instr.func_ptr = ARM::Instructions::AND;
        // instr.mnemonic = fmt::format("and{}{} r{}, r{}, #{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.op2);
        break;
      }
      case EOR: {
        // throw std::runtime_error("EOR");
        instr.func_ptr = ARM::Instructions::EOR;
        // instr.mnemonic = fmt::format("eor{}{} r{}, r{},#{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.op2);
        break;
      }
      case SUB: {
        // instr.print_params();
        instr.func_ptr = ARM::Instructions::SUB;
        // instr.mnemonic = fmt::format("sub{}{} r{}, r{},#{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.op2);
        break;
      }
      case RSB: {
        instr.func_ptr = ARM::Instructions::RSB;
        // instr.mnemonic = fmt::format("rsb{}{} r{}, r{},#{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.op2);
        // assert(0);
        break;
      }
      case ADD: {
        // ADD - Rd:= Op1 + Op2
        instr.func_ptr = ARM::Instructions::ADD;
        // instr.mnemonic = fmt::format("add{}{} r{}, r{}, #{:#x}", condition_map.at(instr.condition), instr.S ? "s" : "", +instr.Rd, +instr.Rn, instr.op2);
        break;
      }
      case ADC: {
        instr.func_ptr = ARM::Instructions::ADC;
        // instr.mnemonic = fmt::format("adc{}{} r{}, r{},#{:#x}", condition_map.at(instr.condition), instr.S ? "s" : "", +instr.Rd, +instr.Rn, instr.op2);
        break;
      }
      case SBC: {
        instr.func_ptr = ARM::Instructions::SBC;
        // instr.mnemonic = fmt::format("sbc{}{} r{}, r{},#{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.op2);
        break;
      }
      case RSC: {
        instr.func_ptr = ARM::Instructions::RSC;
        // instr.mnemonic = fmt::format("rsc{}{} r{}, r{},#{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.op2);
        break;
      }
      case TST: {
        instr.func_ptr = ARM::Instructions::TST;
        // instr.mnemonic = fmt::format("tst{} r{}, #{:#x}", condition_map.at(instr.condition), +instr.Rn, instr.op2);
        break;
      }
      case TEQ: {
        instr.func_ptr = ARM::Instructions::TEQ;
        // instr.mnemonic = fmt::format("teq{} r{}, #{:#x}", condition_map.at(instr.condition), +instr.Rn, instr.op2);
        break;
      }
      case CMP: {
        instr.func_ptr = ARM::Instructions::CMP;
        // instr.mnemonic = fmt::format("cmp{} r{}, #{:#x}", condition_map.at(instr.condition), +instr.Rn, instr.op2);
        break;
      }
      case CMN: {
        instr.func_ptr = ARM::Instructions::CMN;
        // instr.mnemonic = fmt::format("cmn{} r{}, r{}, #${:#x}", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.op2);
        break;
      }
      case ORR: {
        instr.func_ptr = ARM::Instructions::ORR;
        // instr.mnemonic = fmt::format("orr{} r{}, r{}, #${:#x}", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.op2);
        break;
      }
      case MOV: {
        instr.func_ptr = ARM::Instructions::MOV;
        // instr.mnemonic = fmt::format("mov{}{} r{}, #{:#x}", condition_map.at(instr.condition), instr.S ? "s" : "", +instr.Rd, instr.op2);
        break;
      }
      case BIC: {
        instr.func_ptr = ARM::Instructions::BIC;
        // instr.mnemonic = fmt::format("bic{}{} r{}, r{},#{:#x}", instr.S ? "s" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.op2);
        break;
      }
      case MVN: {
        instr.func_ptr = ARM::Instructions::MVN;
        // instr.mnemonic = fmt::format("mvn{}{} r{}, #{:#x}", condition_map.at(instr.condition), instr.S ? "s" : "", +instr.Rd, instr.op2);
        break;
      }

      default: {
        // spdlog::error("could not resolve data processing opcode: {:08b}", o_opcode);
        assert(0);
      }
    }
    return instr;
  }

  if ((instr.opcode & 0xe000000) == 0x4000000) {  // LDR, STR [immediate]
    // SPDLOG_INFO("LDR, STR [immediate]");
    instr.I = (instr.opcode & (1 << 25)) ? 1 : 0;
    instr.P = (instr.opcode & (1 << 24)) ? 1 : 0;
    instr.U = (instr.opcode & (1 << 23)) ? 1 : 0;
    instr.B = (instr.opcode & (1 << 22)) ? 1 : 0;
    instr.W = (instr.opcode & (1 << 21)) ? 1 : 0;
    instr.L = (instr.opcode & (1 << 20)) ? 1 : 0;

    instr.Rn     = (instr.opcode & 0xf0000) >> 16;
    instr.Rd     = (instr.opcode & 0xf000) >> 12;
    instr.offset = (instr.opcode & 0xfff);
    assert(instr.I == 0);
    if (instr.L) {
      instr.func_ptr = ARM::Instructions::LDR;
      // instr.mnemonic = fmt::format("ldr{}{} r{}, [r{}, {}#${:#06x}]", instr.B ? "b" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.U ? "+" : "-", instr.offset);
      // fmt::println("{}", instr.mnemonic);
    } else {
      instr.func_ptr = ARM::Instructions::STR;
      // instr.mnemonic = fmt::format("str{}{} r{}, [r{}, {}#${:#06x}]", instr.B ? "b" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.U ? "+" : "-", instr.offset);
    }

    return instr;
  }

  if ((instr.opcode & 0xe000010) == 0x6000000) {  // LDR [register]
    // SPDLOG_INFO("LDR, STR [register]");
    instr.I = (instr.opcode & (1 << 25)) ? 1 : 0;
    instr.P = (instr.opcode & (1 << 24)) ? 1 : 0;
    instr.U = (instr.opcode & (1 << 23)) ? 1 : 0;
    instr.B = (instr.opcode & (1 << 22)) ? 1 : 0;
    instr.W = (instr.opcode & (1 << 21)) ? 1 : 0;
    instr.L = (instr.opcode & (1 << 20)) ? 1 : 0;

    instr.Rn = (instr.opcode & 0xf0000) >> 16;
    instr.Rd = (instr.opcode & 0xf000) >> 12;

    instr.Rm = (instr.opcode & 0xf);

    instr.shift_amount = (instr.opcode & 0xf80) >> 7;
    instr.shift_type   = (instr.opcode & 0b1100000) >> 5;

    std::string shift_string = get_shift_string(instr.shift_type);

    // 25     I - Immediate Offset Flag (0=Immediate, 1=Shifted Register)
    assert(instr.I == 1);

    if (instr.L) {
      instr.func_ptr = ARM::Instructions::LDR;
      // LDR R3, [R2, -R1, LSL #2]!
      // instr.mnemonic = fmt::format("ldr{}{} r{}, [r{}, {}r{}, {} #{}]", instr.B ? "b" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.U ? "+" : "-", +instr.Rm, shift_string, instr.shift_amount);
    } else {
      instr.func_ptr = ARM::Instructions::STR;
      // instr.mnemonic = fmt::format("str{}{} r{}, [r{}, {}r{}, {} #{}]", instr.B ? "b" : "", condition_map.at(instr.condition), +instr.Rd, +instr.Rn, instr.U ? "+" : "-", +instr.Rm, shift_string, instr.shift_amount);
    }

    // SPDLOG_DEBUG("{}", instr.mnemonic);
    return instr;
    // assert(0);
  }

  if ((instr.opcode & 0xe000000) == 0x8000000) {  // LDM, STM (Block Data Transfer)
    instr.P  = (instr.opcode & (1 << 24)) ? 1 : 0;
    instr.U  = (instr.opcode & (1 << 23)) ? 1 : 0;
    instr.S  = (instr.opcode & (1 << 22)) ? 1 : 0;
    instr.W  = (instr.opcode & 0x200000) ? 1 : 0;
    instr.L  = (instr.opcode & 0x100000) ? 1 : 0;
    instr.Rn = (instr.opcode & 0xf0000) >> 16;

    u16 reg_list_v = instr.opcode & 0xFFFF;

    std::vector<u8> reg_list;  // TODO: reconsider using a different structure here, this keeps getting resized, causing copies and new memory allocations.

    for (size_t idx = 0; idx < 16; idx++) {
      if (reg_list_v & (1 << idx)) { reg_list.push_back(idx); }
    }

    if (instr.L == 0) {  // STM
      instr.func_ptr = ARM::Instructions::STM;
      // instr.mnemonic = fmt::format("stm{} r{}{}, r{}{}", get_addressing_mode(instr), +instr.Rn, instr.W ? "!" : "", fmt::join(reg_list, ",r"), instr.S ? "^" : "");
    } else {
      instr.func_ptr = ARM::Instructions::LDM;
      // instr.mnemonic = fmt::format("ldm{} r{}{}, r{}{}", get_addressing_mode(instr), +instr.Rn, instr.W ? "!" : "", fmt::join(reg_list, ",r"), instr.S ? "^" : "");
    }

    return instr;
  }

  if ((instr.opcode & (0xe000000)) == 0xa000000) {  // B (Branch)
    // spdlog::info("read instruction {:#010X} is of branch", instr.opcode);

    instr.offset = (instr.opcode & 0xFFFFFF);
    instr.L      = (instr.opcode & 0x1000000) != 0;

    int x = instr.offset;

    if (x & 0x800000) { x -= 0x1000000; }
    x *= 4;

    // fmt::println("offset calced: {:#010x}", x);
    instr.offset = x;

    // instr.mnemonic = fmt::format("b{}{} #{:#x}", instr.L ? "l" : "", condition_map.at(instr.condition), regs.r[15] + x + 4);

    instr.func_ptr = ARM::Instructions::B;
    return instr;
  }

  if ((instr.opcode & 0xf000000) == 0xf000000) {  // SWI
    // instr.mnemonic = fmt::format("swi #{:#010x}", (instr.opcode & 0xff0000) >> 16);
    instr.func_ptr = ARM::Instructions::SWI;
    return instr;
  }

  SPDLOG_DEBUG("[ARM] failed to decode: {:#010x} PC: {:#010x}", instr.opcode, regs.r[15]);
  return instr;
};