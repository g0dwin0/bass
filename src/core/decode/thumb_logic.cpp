...
InstructionInfo ARM7TDMI::thumb_decode(InstructionInfo& instr) {
  ...
  if ((instr.opcode & 0xfc00) == 0x4400) {  // ADD, CMP, MOV (high registers)
    ...
    u8 MSBd = ((instr.opcode & 1 << 7) ? 8 : 0);
    u8 MSBs = ((instr.opcode & 1 << 6) ? 8 : 0);

    instr.Rd = MSBd + (instr.opcode & 0b111);
    instr.Rs = MSBs + ((instr.opcode & 0b111000) >> 3);
  ...
  if ((instr.opcode & 0xff00) == 0xb000) {  // ADD, SUB (SP)
    ...
    instr.imm = (instr.opcode & 0b1111111) * 4;
    instr.op2 = instr.imm;
  ...
  if ((instr.opcode & 0xf000) == 0xd000) {  // (conditional branching)
    ...
    instr.empty = true;  // check condition
                         // fmt::println("ticked");
    instr.offset = ((instr.opcode & 0xff));
    int x        = instr.offset;
    ...
    // printf("%d\n", instr.L);
    // printf("%s\n", condition_map.at(instr.condition).c_str());
    // printf("%08X\n", regs.r[15] + instr.offset + 2);
    // printf("%d\n", instr.L);
    // printf("%d\n", instr.L);
